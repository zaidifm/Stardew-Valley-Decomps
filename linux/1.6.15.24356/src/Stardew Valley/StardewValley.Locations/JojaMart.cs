		...
		if (doesTileHaveProperty(tileLocation.X, tileLocation.Y, "Action", "Buildings") == "JoinJoja")
		{
			...
			else if (!Game1.player.mailReceived.Contains("JojaMember"))
			{
				...
				else if (Game1.stats.DaysPlayed < 0)
				{
					...
					Morris.setNewDialogue(translationKey);
					Game1.drawDialogue(Morris);
				}
				else
				{
					string text = ((Game1.dayOfMonth % 7 == 0 || Game1.dayOfMonth % 7 == 6) ? "Data\\ExtraDialogue:Morris_WeekendGreeting" : "Data\\ExtraDialogue:Morris_FirstGreeting");
					if (Game1.IsMasterGame)
					{
						if (!Game1.player.eventsSeen.Contains("611439"))
						{
							Morris.setNewDialogue(text);
							Game1.drawDialogue(Morris);
						}
						else if (Game1.player.mailReceived.Contains("ccIsComplete"))
						{
							Morris.setNewDialogue(text + "_CommunityCenterComplete");
							Game1.drawDialogue(Morris);
						}
						else
						{
							Morris.setNewDialogue(Dialogue.FromTranslation(Morris, text + "_MembershipAvailable", 5000));
					...
					{
						Morris.setNewDialogue(text + "_SecondPlayer");
						Game1.drawDialogue(Morris);
					}
				}
			}
