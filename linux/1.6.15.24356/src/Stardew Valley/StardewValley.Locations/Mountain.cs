...
public class Mountain : GameLocation
{
	...
	public void ApplyTreehouseIfNecessary()
	{
		...
		if (((obj != null && obj.whichEvent.Value == 14) || Game1.MasterPlayer.mailReceived.Contains("leoMoved") || Game1.MasterPlayer.mailReceived.Contains("leoMoved%&NL&%")) && !treehouseBuilt)
		{
			...
			layer.Tiles[16, 6] = new StaticTile(layer, tileSheet, BlendMode.Alpha, 197);
			layer.Tiles[16, 7] = new StaticTile(layer, tileSheet, BlendMode.Alpha, 213);
			layer2.Tiles[16, 8] = new StaticTile(layer2, tileSheet, BlendMode.Alpha, 229);
			layer.Tiles[16, 7].Properties["Action"] = "LockedDoorWarp 3 8 LeoTreeHouse 600 2300";
			treehouseBuilt = true;
	...
	private void restoreBridge()
	{
		...
		Map obj = localizedContentManager.Load<Map>("Maps\\Mountain-BridgeFixed");
		int num = 92;
		int num2 = 24;
		Layer layer = map.RequireLayer("Back");
		Layer layer2 = map.RequireLayer("Buildings");
		Layer layer3 = map.RequireLayer("Front");
		Layer layer4 = obj.RequireLayer("Back");
		Layer layer5 = obj.RequireLayer("Buildings");
		Layer layer6 = obj.RequireLayer("Front");
		TileSheet tileSheet = map.RequireTileSheet(0, "outdoors");
		for (int i = 0; i < layer4.LayerWidth; i++)
		{
			for (int j = 0; j < layer4.LayerHeight; j++)
			{
				layer.Tiles[i + num, j + num2] = ((layer4.Tiles[i, j] == null) ? null : new StaticTile(layer, tileSheet, BlendMode.Alpha, layer4.Tiles[i, j].TileIndex));
				layer2.Tiles[i + num, j + num2] = ((layer5.Tiles[i, j] == null) ? null : new StaticTile(layer2, tileSheet, BlendMode.Alpha, layer5.Tiles[i, j].TileIndex));
				layer3.Tiles[i + num, j + num2] = ((layer6.Tiles[i, j] == null) ? null : new StaticTile(layer3, tileSheet, BlendMode.Alpha, layer6.Tiles[i, j].TileIndex));
			}
		}
		bridgeRestored = true;
	...
	protected override void resetSharedState()
	{
		...
		Vector2 key = new Vector2(29f, 9f);
		if (!objects.ContainsKey(key))
		{
			objects.Add(key, new Torch("146", bigCraftable: true)
			{
				IsOn = false,
				Fragility = 2
			});
			objects[key].checkForAction(null);
		}
	...
	private void quarryDayUpdate()
	{
		...
		int num = Math.Min(16, 5 + Game1.year * 2);
		for (int i = 0; i < num; i++)
		{
			...
			{
				terrainFeatures.Add(randomPositionInThisRectangle, new Tree((1 + Game1.random.Next(2)).ToString(), 1));
			}
			else if (Game1.random.NextDouble() < 0.02)
			{
				if (Game1.random.NextDouble() < 0.1)
				{
					objects.Add(randomPositionInThisRectangle, new Object(46.ToString(), 1)
					{
				...
				{
					objects.Add(randomPositionInThisRectangle, new Object(((Game1.random.Next(7) + 1) * 2).ToString(), 1)
					{
						MinutesUntilReady = 5
					});
				}
			...
			{
				objects.Add(randomPositionInThisRectangle, ItemRegistry.Create<Object>(Game1.random.NextBool(0.15) ? "(O)SeedSpot" : "(O)590"));
			}
			else if (Game1.random.NextDouble() < 0.15)
			{
				if (Game1.random.NextDouble() < 0.001)
				{
					objects.Add(randomPositionInThisRectangle, new Object("765", 1)
					{
				...
				{
					objects.Add(randomPositionInThisRectangle, new Object("764", 1)
					{
						MinutesUntilReady = 8
					});
				}
				...
				{
					objects.Add(randomPositionInThisRectangle, new Object("290", 1)
					{
						MinutesUntilReady = 5
					});
				}
				...
				{
					objects.Add(randomPositionInThisRectangle, new Object("751", 1)
					{
						MinutesUntilReady = 3
					});
				}
			...
			{
				objects.Add(randomPositionInThisRectangle, new Object(Game1.random.Choose("BasicCoalNode0", "BasicCoalNode1"), 1)
				{
					MinutesUntilReady = 5
				});
			}
	...
	public override bool isTilePlaceable(Vector2 tileLocation, bool itemIsPassable = false)
	{
		...
		if (landslide.Value && landSlideRect.Contains(value))
		{
			return false;
		}
		if (railroadAreaBlocked.Value && railroadBlockRect.Contains(value))
		{
			return false;
		}
