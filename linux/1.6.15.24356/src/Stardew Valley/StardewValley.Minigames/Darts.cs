...
public class Darts : IMinigame
{
	...
	public bool tick(GameTime time)
	{
		...
		case GameState.Firing:
			if (hangTime > 0f)
			{
				...
				if (hangTime <= 0f)
				{
					float num2 = Utility.RandomFloat(0f, (float)Math.PI * 2f);
					aimPosition += new Vector2((float)Math.Sin(num2), (float)Math.Cos(num2)) * Utility.RandomFloat(0f, GetRadiusFromCharge() * 32f);
	...
	public void draw(SpriteBatch b)
	{
		...
		{
			SpriteText.drawStringWithScrollCenteredAt(b, alternateTextString, (int)(vector.X + vector2.X), (int)(vector.Y + vector2.Y), "", 1f, SpriteText.color_Red);
		}
		else if (points >= 0)
		{
			...
			if (currentGameState == GameState.ShowScore || currentGameState == GameState.Scoring)
			{
				...
				vector.Y += 64f;
				string s2 = ((currentGameState == GameState.ShowScore) ? (" " + lastHitString + " ") : (" " + lastHitAmount + " "));
				SpriteText.drawStringWithScrollCenteredAt(b, s2, (int)(vector.X + vector2.X), (int)(vector.Y + vector2.Y), "", 1f, SpriteText.color_Blue, 2);
			}
		}
		...
		if (gameOverString != "")
		{
			...
			if (points == 0)
			{
				...
				if (IsPerfectVictory())
				{
					SpriteText.drawStringWithScrollCenteredAt(b, Game1.content.LoadString("Strings\\UI:Darts_WinTextPerfect", throwsCount), (int)(vector.X + vector2.X), (int)(vector.Y + vector2.Y), "", 1f, SpriteText.color_Blue, 2);
				}
				else
				{
					SpriteText.drawStringWithScrollCenteredAt(b, Game1.content.LoadString("Strings\\UI:Darts_WinText", throwsCount), (int)(vector.X + vector2.X), (int)(vector.Y + vector2.Y), "", 1f, SpriteText.color_Blue, 2);
				}
			}
			else
			{
