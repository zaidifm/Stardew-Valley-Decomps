...
public class ParrotPlatform
{
	...
	public class Parrot
	{
		...
		public virtual void Draw(SpriteBatch b)
		{
			...
			{
				b.Draw(_platform.texture, vector6, new Microsoft.Xna.Framework.Rectangle(16, 68, 16, 16), Color.White, 0f, new Vector2(8f, 8f), 4f, SpriteEffects.None, num7 + num6);
				num6 += num5;
				if (vector5.HasValue)
				{
					Vector2 vector7 = vector6 - vector5.Value;
					int num8 = (int)Math.Ceiling(vector7.Length() / 4f);
					float rotation = 0f - (float)Math.Atan2(vector7.X, vector7.Y) + (float)Math.PI / 2f;
	...
	public virtual void Update(GameTime time)
	{
		...
		{
			bool flag = new Microsoft.Xna.Framework.Rectangle((int)position.X / 64, (int)position.Y / 64, 3, 1).Contains(Game1.player.TilePoint);
	...
	public virtual bool AnswerQuestion(Response answer)
	{
		if (this == activePlatform)
		{
			...
			{
				string text = answer.responseKey.Substring(2);
				foreach (KeyValuePair<string, KeyValuePair<string, Point>> destination in GetDestinations())
				{
					if (destination.Key == text)
					{
