	...
	public override bool update(GameTime time, GameLocation environment)
	{
		...
		case 0:
			if (sprite.CurrentAnimation == null)
			{
				...
				int num3 = Game1.random.Next(1, 5);
		...
		case 4:
			...
			{
				int num = (flip ? 1 : (-1));
				if (!environment.isCollidingPosition(getBoundingBox(num, 0), Game1.viewport, isFarmer: false, 0, glider: false, null, pathfinding: false, projectile: false, ignoreCharacterRequirement: true))
				{
					position.X += num;
				}
			}
			...
			{
				float num2 = (flip ? 0.5f : (-0.5f));
				if (Math.Abs(position.X + num2 - startingPosition.X) < 8f)
				{
					position.X += num2;
				}
				else
