...
public class AmbientLocationSounds
{
	...
	public static void update(GameTime time)
	{
		...
		if (volumeOverrideForLocChange >= 0f)
		{
			for (int j = 0; j < shortestDistanceForCue.Length; j++)
			{
				...
				{
					float num2 = Math.Min(volumeOverrideForLocChange, Math.Min(1f, 1f - shortestDistanceForCue[j] / ((float)farthestSoundDistance * 1.5f)));
					num2 = (float)Math.Pow(num2, 5.0);
					switch (j)
					{
					case 0:
						if (babblingBrook != null)
					...
					case 4:
						if (waterfall != null)
						{
							waterfall.SetVariable("Volume", num2 * 100f * Math.Min(Game1.ambientPlayerVolume, Game1.options.ambientVolumeLevel));
							waterfall.Resume();
						}
						break;
					case 5:
						if (waterfallBig != null)
						{
							waterfallBig.SetVariable("Volume", num2 * 100f * Math.Min(Game1.ambientPlayerVolume, Game1.options.ambientVolumeLevel));
							waterfallBig.Resume();
						}
						break;
					}
	...
	{
		if (sounds.TryGetValue(tileLocation * 64f, out var value))
		{
			switch (value)
			{
			case 0:
