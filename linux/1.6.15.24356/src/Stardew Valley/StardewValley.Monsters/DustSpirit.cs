...
public class DustSpirit : Monster
{
	...
	public override void draw(SpriteBatch b)
	{
		if (!base.IsInvisible && Utility.isOnScreen(base.Position, 128))
		{
			...
			b.Draw(Sprite.Texture, getLocalPosition(Game1.viewport) + new Vector2(32 + ((shakeTimer > 0) ? Game1.random.Next(-1, 2) : 0), 64 + yJumpOffset), Sprite.SourceRect, Color.White, rotation, new Vector2(8f, 16f), new Vector2(scale.Value + (float)Math.Max(-0.1, (double)(yJumpOffset + 32) / 128.0), scale.Value - Math.Max(-0.1f, (float)yJumpOffset / 256f)) * 4f, flip ? SpriteEffects.FlipHorizontally : SpriteEffects.None, Math.Max(0f, drawOnTop ? 0.991f : ((float)y / 10000f)));
			if (isGlowing)
			{
				b.Draw(Sprite.Texture, getLocalPosition(Game1.viewport) + new Vector2(32f, 64 + yJumpOffset), Sprite.SourceRect, glowingColor * glowingTransparency, rotation, new Vector2(8f, 16f), Math.Max(0.2f, scale.Value) * 4f, flip ? SpriteEffects.FlipHorizontally : SpriteEffects.None, Math.Max(0f, drawOnTop ? 0.99f : ((float)y / 10000f + 0.001f)));
			}
			b.Draw(Game1.shadowTexture, getLocalPosition(Game1.viewport) + new Vector2(32f, 80f), Game1.shadowTexture.Bounds, Color.White, 0f, new Vector2(Game1.shadowTexture.Bounds.Center.X, Game1.shadowTexture.Bounds.Center.Y), 4f + (float)yJumpOffset / 64f, SpriteEffects.None, (float)(y - 1) / 10000f);
		}
	}
	...
	{
		if (base.currentLocation != null && base.currentLocation.terrainFeatures.TryGetValue(base.Tile, out var value) && value is Grass grass)
		{
	...
	public override void behaviorAtGameTick(GameTime time)
	{
		...
		if (yJumpOffset == 0)
		{
			if (Game1.random.NextDouble() < 0.01)
			{
				...
				Game1.multiplayer.broadcastSprites(base.currentLocation, new TemporaryAnimatedSprite("TileSheets\\animations", new Rectangle(0, 128, 64, 64), 40f, 4, 0, standingPosition + new Vector2(-21f, 0f), flicker: false, flipped: false)
				{
					layerDepth = (standingPosition.Y - 10f) / 10000f
				});
				foreach (Vector2 adjacentTileLocation in Utility.getAdjacentTileLocations(base.Tile))
				{
					if (base.currentLocation.objects.TryGetValue(adjacentTileLocation, out var value) && (value.IsBreakableStone() || value.IsTwig()))
					{
