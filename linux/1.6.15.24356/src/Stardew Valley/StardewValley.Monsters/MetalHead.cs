...
public class MetalHead : Monster
{
	...
	public override void draw(SpriteBatch b)
	{
		if (!base.IsInvisible && Utility.isOnScreen(base.Position, 128))
		{
			...
			b.Draw(Sprite.Texture, getLocalPosition(Game1.viewport) + new Vector2(32f, 48 + yJumpOffset), Sprite.SourceRect, c.Value, rotation, new Vector2(8f, 16f), Math.Max(0.2f, scale.Value) * 4f, flip ? SpriteEffects.FlipHorizontally : SpriteEffects.None, Math.Max(0f, drawOnTop ? 0.991f : ((float)y / 10000f)));
		}
	}
	...
	public override List<Item> getExtraDropItems()
	{
		...
		{
			list.Add(ItemRegistry.Create("(H)51"));
		}
		return list;
	}
