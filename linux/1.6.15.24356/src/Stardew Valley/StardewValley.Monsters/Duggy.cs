...
public class Duggy : Monster
{
	...
	public override void draw(SpriteBatch b)
	{
		if (!base.IsInvisible && Utility.isOnScreen(base.Position, 128))
		{
			...
			b.Draw(Sprite.Texture, getLocalPosition(Game1.viewport) + new Vector2(32f, boundingBox.Height / 2 + yJumpOffset), Sprite.SourceRect, Color.White, rotation, new Vector2(8f, 16f), Math.Max(0.2f, scale.Value) * 4f, flip ? SpriteEffects.FlipHorizontally : SpriteEffects.None, Math.Max(0f, drawOnTop ? 0.991f : ((float)y / 10000f)));
			if (isGlowing)
			{
				b.Draw(Sprite.Texture, getLocalPosition(Game1.viewport) + new Vector2(32f, boundingBox.Height / 2 + yJumpOffset), Sprite.SourceRect, glowingColor * glowingTransparency, rotation, new Vector2(8f, 16f), Math.Max(0.2f, scale.Value) * 4f, flip ? SpriteEffects.FlipHorizontally : SpriteEffects.None, Math.Max(0f, drawOnTop ? 0.991f : ((float)y / 10000f + 0.001f)));
			}
		}
	}
	...
	public override void behaviorAtGameTick(GameTime time)
	{
		...
		if (Sprite.currentFrame < 4)
		{
			...
			if (!base.IsInvisible || boundingBox.Contains(base.Player.StandingPixel))
			{
				if (base.IsInvisible)
				{
		...
		if (Sprite.currentFrame >= 10)
		{
			...
			base.currentLocation.map.RequireLayer("Back").Tiles[tilePoint.X, tilePoint.Y].TileIndex = 0;
			base.currentLocation.removeObjectsAndSpawned(tilePoint.X, tilePoint.Y, 1, 1);
			base.DamageToFarmer = 0;
		}
	}
}
