...
public class FlyingCompanion : Companion
{
	...
	public override void Draw(SpriteBatch b)
	{
		...
		if (perching)
		{
			...
			{
				b.Draw(texture, Game1.GlobalToLocal(base.Position + base.Owner.drawOffset + new Vector2(0f, (0f - height) * 4f) + extraPosition), new Rectangle((int)(parrot_squatTimer % 1000f) / 500 * 16 + 128, startingYForVariant.Value, 16, 16), Color.White, 0f, new Vector2(8f, 8f), 4f, effects, _position.Y / 10000f);
			}
			else if (parrot_squawkTimer > 0f)
			{
				b.Draw(texture, Game1.GlobalToLocal(base.Position + base.Owner.drawOffset + new Vector2(0f, (0f - height) * 4f) + extraPosition), new Rectangle(160, startingYForVariant.Value, 16, 16), Color.White, 0f, new Vector2(8f, 8f), 4f, effects, _position.Y / 10000f);
			}
			else
			{
				b.Draw(texture, Game1.GlobalToLocal(base.Position + base.Owner.drawOffset + new Vector2(0f, (0f - height) * 4f) + extraPosition), new Rectangle(128, startingYForVariant.Value, 16, 16), Color.White, 0f, new Vector2(8f, 8f), 4f, effects, _position.Y / 10000f);
			}
		}
		else
		{
			b.Draw(texture, Game1.GlobalToLocal(base.Position + base.Owner.drawOffset + new Vector2(0f, (0f - height) * 4f) + extraPosition), new Rectangle(whichSubVariant.Value * 64 + (int)(flitTimer / (float)(500 / flapAnimationLength)) * 16, startingYForVariant.Value, 16, 16), Color.White, 0f, new Vector2(8f, 8f), 4f, effects, _position.Y / 10000f);
	...
	public override void InitializeCompanion(Farmer farmer)
	{
		...
		{
			Random random = Utility.CreateRandom(farmer.uniqueMultiplayerID.Value);
			whichSubVariant.Value = random.Next(4);
			if (whichVariant.Value == 0 && random.NextDouble() < 0.5)
			{
