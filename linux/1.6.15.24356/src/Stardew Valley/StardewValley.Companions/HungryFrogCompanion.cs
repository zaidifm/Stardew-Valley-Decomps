...
public class HungryFrogCompanion : HoppingCompanion
{
	...
	public override void Update(GameTime time, GameLocation location)
	{
		...
		if (base.IsLocal)
		{
			...
			if (monsterEatCheckTimer >= 2000f && fullnessTime <= 0f && !tongueOut.Value)
			{
				...
				{
					Monster monster = Utility.findClosestMonsterWithinRange(location, base.Position, 300);
					if (monster != null)
					{
						if (monster is Bat && monster.Age == 789)
						{
							monsterEatCheckTimer = 0f;
							return;
						}
						if (monster.Name.Equals("Truffle Crab"))
						{
							monsterEatCheckTimer = 0f;
							return;
						}
						if (monster is GreenSlime greenSlime && greenSlime.prismatic.Value)
						{
							monsterEatCheckTimer = 0f;
							return;
						}
						height = 0f;
						Vector2 velocityTowardPoint = Utility.getVelocityTowardPoint(base.Position, monster.getStandingPosition(), 12f);
						...
						direction.Value = ((!(monster.Position.X < base.Position.X)) ? 1 : 3);
					}
				}
				tongueOutTimer = 0f;
			}
			if (tongueOut.Value)
			{
				...
				if (attachedMonster == null)
				{
					...
					{
						int num = 40;
						Rectangle r = new Rectangle((int)tonguePosition.X + 32 - num / 2, (int)tonguePosition.Y + 32 - num / 2, num, num);
						if (base.Owner.currentLocation.doesPositionCollideWithCharacter(r) is Monster m)
						{
				...
				{
					Vector2 value = Vector2.Subtract(base.Position + new Vector2(-32f, -32f) + new Vector2((direction.Value != 3) ? 28 : 0, -20f), tonguePosition.Value);
					value.Normalize();
					value *= 12f;
					tongueVelocity.Value = value;
				}
	...
	public override void Draw(SpriteBatch b)
	{
		...
		Rectangle rectangle = new Rectangle((fullnessTime > 0f) ? 128 : 0, 16 + whichVariant.Value * 16, 16, 16);
		Color color = ((whichVariant.Value == 7) ? Utility.GetPrismaticColor() : Color.White);
		...
		if (tongueOut.Value)
		{
			b.Draw(texture, Game1.GlobalToLocal(base.Position + base.Owner.drawOffset + new Vector2(0f, (0f - height) * 4f)), Utility.translateRect(rectangle, 112), color, 0f, new Vector2(8f, 16f), 4f, effects, (_position.Y - 12f) / 10000f);
		}
		else if (height > 0f)
		{
			if (gravity > 0f)
			{
				b.Draw(texture, Game1.GlobalToLocal(base.Position + base.Owner.drawOffset + new Vector2(0f, (0f - height) * 4f)), Utility.translateRect(rectangle, 16), color, 0f, new Vector2(8f, 16f), 4f, effects, (_position.Y - 12f) / 10000f);
			}
			else if (gravity > -0.15f)
			{
				b.Draw(texture, Game1.GlobalToLocal(base.Position + base.Owner.drawOffset + new Vector2(0f, (0f - height) * 4f)), Utility.translateRect(rectangle, 32), color, 0f, new Vector2(8f, 16f), 4f, effects, (_position.Y - 12f) / 10000f);
			}
			else
			{
				b.Draw(texture, Game1.GlobalToLocal(base.Position + base.Owner.drawOffset + new Vector2(0f, (0f - height) * 4f)), Utility.translateRect(rectangle, 48), color, 0f, new Vector2(8f, 16f), 4f, effects, (_position.Y - 12f) / 10000f);
			}
		}
		else if (lastHopTimer > 5000f && !tongueOut.Value)
		{
			b.Draw(texture, Game1.GlobalToLocal(base.Position + base.Owner.drawOffset + new Vector2(0f, (0f - height) * 4f)), Utility.translateRect(rectangle, 80 + ((Game1.currentGameTime.TotalGameTime.TotalMilliseconds % 400.0 >= 200.0) ? 16 : 0)), color, 0f, new Vector2(8f, 16f), 4f, effects, (_position.Y - 12f) / 10000f);
		}
		else
		{
			b.Draw(texture, Game1.GlobalToLocal(base.Position + base.Owner.drawOffset + new Vector2(0f, (0f - height) * 4f)), rectangle, color, 0f, new Vector2(8f, 16f), 4f, effects, (_position.Y - 12f) / 10000f);
		}
		...
		if (tongueOut.Value)
		{
			Vector2 vector = Game1.GlobalToLocal(tonguePosition.Value + new Vector2(32f));
			Vector2 vector2 = Game1.GlobalToLocal(base.Position + new Vector2(-32f, -32f) + new Vector2((direction.Value != 3) ? 44 : 24, 16f));
			Utility.drawLineWithScreenCoordinates((int)vector2.X, (int)vector2.Y, (int)vector.X, (int)vector.Y, b, Color.Red, 1f, 4);
			Texture2D projectileSheet = Projectile.projectileSheet;
			Rectangle sourceRectForStandardTileSheet = Game1.getSourceRectForStandardTileSheet(Projectile.projectileSheet, 19, 16, 16);
			b.Draw(projectileSheet, Game1.GlobalToLocal(tonguePosition.Value + new Vector2(32f, 32f)) + base.Owner.drawOffset, sourceRectForStandardTileSheet, Color.White, 0f, new Vector2(8f, 8f), 4f, SpriteEffects.None, 1f);
		}
	}
}
