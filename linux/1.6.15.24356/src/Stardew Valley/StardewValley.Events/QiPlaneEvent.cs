...
public class QiPlaneEvent : BaseFarmEvent
{
	...
	public override void draw(SpriteBatch b)
	{
		...
		if (qiPlanePos.X > (float)(Game1.graphics.GraphicsDevice.Viewport.Width - 480))
		{
			...
			if (textTimer % 100f < num % 100f && (int)(textTimer / 100f) < str.Length)
			{
	...
	public override bool tickUpdate(GameTime time)
	{
		...
		{
			tempSprites[num].update(time);
			if (tempSprites[num].motion.Y < 1f)
			{
				tempSprites[num].motion.Y = 1f;
			}
			if (tempSprites[num].position.Y > (float)(Game1.graphics.GraphicsDevice.Viewport.Height + 500))
			{
				tempSprites[num].alphaFade = 0.01f;
			}
			if (tempSprites[num].alpha <= 0f)
			{
